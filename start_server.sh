...
# Target the physical Wi-Fi interface (wlan0)
INTERFACE="wlan0"

echo "🔓 Opening Android Firewall for Local LAN DNS traffic..."
sudo iptables -A INPUT -i $INTERFACE -p udp --dport 53 -j ACCEPT 2>/dev/null
sudo iptables -A INPUT -i $INTERFACE -p tcp --dport 53 -j ACCEPT 2>/dev/null
sudo iptables -t raw -A PREROUTING -i $INTERFACE -p udp --dport 53 -j NOTRACK 2>/dev/null
sudo iptables -t raw -A PREROUTING -i $INTERFACE -p tcp --dport 53 -j NOTRACK 2>/dev/null
